...
bool ForceSensorNode::openSerialPort(const std::string & port, int baudrate)
{
  ...
  if (tcgetattr(serial_fd_, &tty) != 0) {
    RCLCPP_ERROR(
      this->get_logger(), "Error getting terminal attributes: %s",
      strerror(errno));
    return false;
  }
  ...
  switch (baudrate) {
    ...
    default:
      RCLCPP_ERROR(
        this->get_logger(), "Unsupported baudrate: %d",
        baudrate);
      return false;
  }
  ...
  if (tcsetattr(serial_fd_, TCSANOW, &tty) != 0) {
    RCLCPP_ERROR(
      this->get_logger(), "Error setting terminal attributes: %s",
      strerror(errno));
    return false;
  } else {
    RCLCPP_INFO(
      this->get_logger(), "Serial port \"%s\" successfully open",
      serial_port_.c_str());
  }
...
void ForceSensorNode::publish_forces(const std::vector<std::pair<int, float>> & data)
{
  ...
  if (verbose_) {
    RCLCPP_INFO(
      this->get_logger(), "Published force vector of size: %zu",
      msg.data.size());
  }
}
